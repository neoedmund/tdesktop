...
int ReadRotationFromMetadata(not_null<AVStream*> stream) {
	// const auto displaymatrix = av_stream_get_side_data(stream,AV_PKT_DATA_DISPLAYMATRIX,nullptr);
	// New (replace with this):
	const AVPacketSideData *sd = av_packet_side_data_get(
	    stream->codecpar->coded_side_data,
	    stream->codecpar->nb_coded_side_data,
	    AV_PKT_DATA_DISPLAYMATRIX);
	const auto displaymatrix = sd ? reinterpret_cast<const int32_t*>(sd->data) : nullptr;
	auto theta = 0;
	if (displaymatrix) {
